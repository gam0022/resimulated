...
uniform float gVolumeStepSize;  // 0.1 0 1
...
#define saturation 0.850
    ...
    for (int r = 0; r < volsteps; r++) {
        if (s > intersection.distance) break;

        vec3 p = ray.origin + s * ray.direction * .5;
    ...
    intersection.color += clamp(0.01 * v, 0.0, 0.4);
}
