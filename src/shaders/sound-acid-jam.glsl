...
// srtuss, 2014
//
// Writing this crude 303 emulation + some percussion was the first thing i
// tried with the new soundshader-feature. The values are all tweaked by ear.
...
vec2 synth(float tseq, float t) {
    ...
    for (int i = 0; i < NSPC; i++) {
        float h = float(i + 1);
        float inten = 1.0 / h;
        // inten *= sin((pow(h, sin(t) * 0.5 + 0.5) + t * 0.5) * pi2) * 0.9 +
        // 0.1;
...
vec2 mainSound(float time) {
    ...
    // mx += dist(fract(tb / 16.0) * sin(ntof(77.0 - 36.0) * pi2 * time), 8.0) *
    // 0.2; mx += expl(tb) * 0.5;
